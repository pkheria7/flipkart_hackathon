...
Dry-run .eml files are written to data/outputs/eml/<run_id>/ subfolders
so each run's outbox is isolated and auditable.
"""
...
def _run_id_from_plan(plan: dict) -> Optional[str]:
    return plan.get("run_id")


def _format_officer_email(
    officer_id: str, name: str, station: str, assignments: list, date: str
) -> tuple[str, str]:
    ...
    for i, a in enumerate(assignments, 1):
        ...
        if a.get("explanation_kn"):
            lines.append(f"   ಏಕೆ:      {a['explanation_kn']}")
        lines.append(f"   Location: {a['centroid_lat']:.6f}, {a['centroid_lng']:.6f}")
        if a.get("infra_escalation_ready"):
            lines.append(f"   ** INFRA ESCALATION: {a.get('infra_dominant_cause')} — "
                         f"notify {a.get('recommended_agency', 'JOINT_BBMP_BTP')} **")
        lines.append("")
...
def _format_truck_email(
    truck_id: str, driver: str, station: str, assignments: list, date: str
) -> tuple[str, str]:
    ...
    for i, a in enumerate(assignments, 1):
        lines.append(
            f"{i}. {a['time_window']} — {a['cluster_id']} — Officer {a['officer_name']}"
        )
        lines.append(f"   Location: {a['centroid_lat']:.6f}, {a['centroid_lng']:.6f}")
        lines.append("")
    return subject, "\n".join(lines)
...
def _format_head_email(plan: dict) -> tuple[str, str]:
    ...
    lines = [
        ...
        f"Routing source:    {plan.get('routing_source', 'unknown')}",
        f"M10 VRP wired:     {'yes' if plan.get('m10_wired') else 'no'}",
        f"M15 Infra wired:   {'yes' if plan.get('m15_wired') else 'no'}",
        "",
        "Summary by station:",
    ]
    for sp in plan["stations"]:
        lines.append(f"  - {sp['station']}: {sp['summary']} [{sp['routing_source']}]")
    lines.extend([
        "",
        "Please review and approve or revise the plan.",
        "In production, this email would contain Approve/Revise links.",
    ])
    return subject, "\n".join(lines)
...
def dispatch_head_officer(plan: dict, dry_run: bool = True) -> dict:
    ...
    result = send_email(head_email, subject, body, dry_run=dry_run, run_id=run_id)
...
def dispatch_approved_plan(plan: dict, dry_run: bool = True) -> list[dict]:
    ...
    for oid, data in officer_map.items():
        ...
        subject, body = _format_officer_email(
            oid, data["name"], data["station"], data["assignments"], plan["date"]
        )
        result = send_email(data["email"], subject, body, dry_run=dry_run, run_id=run_id)
    ...
    for tid, data in truck_map.items():
        ...
        subject, body = _format_truck_email(
            tid, data["driver"], data["station"], data["assignments"], plan["date"]
        )
        result = send_email(email, subject, body, dry_run=dry_run, run_id=run_id)
...
def dispatch_full_workflow(plan: dict, dry_run: bool = True) -> dict:
    ...
    return {
        ...
        "note": (
            "Officer/truck dispatch requires head-officer approval via "
            "approval_queue.approve_plan()"
        ),
    }
